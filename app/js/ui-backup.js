...
export function buildBackupSection() {
  ...
  /* התמונות נאמרות כאן במפורש. הן יושבות ב-IndexedDB ולא בקובץ הגיבוי
     ...
     ── והמשפט הפותח תלוי בסנכרון ──────────────────────────────────
     "אין שרת שישחזר" היה נכון כל עוד לא היה סשן, והוא מפסיק להיות נכון
     בשנייה שמתחברים. להשאיר אותו שם היה מפחיד את מי שכבר מסונכרן, וגם
     ללמד אותו לא להאמין לטקסט שליד הכפתור. מה שכן נשאר נכון בשני
     המצבים הוא שהתמונות אינן בקובץ, ושהקובץ הוא היחיד שאפשר להחזיק
     ביד. */
  note.textContent = signedIn()
    ? "הקובץ הוא עותק שיושב אצלך, ולא תלוי בסנכרון או בחשבון. הוא כולל את התוכנית, משק הבית, המנות והמזווה — תמונות מנה לא נכנסות אליו."
