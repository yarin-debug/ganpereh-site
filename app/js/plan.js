...
/**
 * הארוחות שיוצגו ליום מסוים, לפי מה שמשק הבית בחר לתכנן.
 *
 * ── ארוחה שכבר תוכננה מוצגת תמיד ────────────────────────────────────
 * גם כשהיא מחוץ להעדפה. הסתרה שלה הייתה יוצרת נתונים בלתי נראים:
 * המנה ממשיכה להיספר במצב היום, ברשימת הקניות ובמאקרו, ולמשתמש אין
 * שום מסך שבו הוא יכול לגעת בה או למחוק אותה. זה בדיוק הכלל של מצרך
 * לא מזוהה שמוצג ברשימה במקום להישמט בשקט.
 *
 * ולכן גם אין כאן שינוי בלוגיקה של dayState ושל הרצף: הן מסננות
 * ארוחות ריקות ממילא, כך שארוחה שלא מתכננים פשוט אינה קיימת בעיניהן.
 * ההעדפה היא סינון תצוגה בלבד.
 */
export function visibleMeals(slots, isoDate, prefMeals) {
  const wanted = new Set(prefMeals?.length ? prefMeals : MEALS.map((meal) => meal.id));
  return dayMeals(slots, isoDate).filter(
    (entry) => wanted.has(entry.meal) || entry.state !== "empty",
  );
}
