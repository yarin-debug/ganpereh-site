...
/**
 * מנתק את המכשיר ממשק הבית. נקרא בהתנתקות.
 *
 * ── למה זה לא מיותר ליד signOut ─────────────────────────────────────
 * `signOut` מוחק את הסשן בלבד, ומזהה משק הבית יושב במפתח אחר.
 * `ensureHousehold` מחזיר מזהה שמור **בלי לבדוק חברות**, ולכן אדם שני
 * שיתחבר במכשיר הזה היה יורש את משק הבית של הראשון: כל משיכה חוזרת
 * ריקה, כל דחיפה נדחית ב-RLS, והמסך מציג "ההתחברות פגה" בלולאה על
 * סשן תקין לגמרי. תקלה שנראית כמו בעיית התחברות ואינה כזו.
 *
 * הטביעות נמחקות יחד איתו מאותה סיבה שהן נמחקות ב-`adoptHousehold`:
 * מול משק בית שטרם ידוע כל מפתח מקומי הוא שינוי שטרם נשלח.
 *
 * הנתונים במכשיר עצמם אינם נוגעים — הם היו כאן לפני החשבון.
 */
 ...
 *
 * ── שני שערים, ולא אחד ──────────────────────────────────────────────
 * `config.js` כבר נושא כתובת פרויקט ומפתח, ולכן `syncConfigured()`
 * מחזיר true והמאזינים כאן **כן** נרשמים. מה שמונע כל בקשת רשת הוא
 * השער השני: `scheduleSync` ו-`syncNow` יוצאים על `signedIn()`, ואין
 * סשן כל עוד לא נבנה מסך התחברות. כלומר "אפס בקשות רשת" נכון היום,
 * אבל לא בגלל התצורה — בגלל היעדר הסשן. אל תסמוך על הראשון לבדו.
 */
